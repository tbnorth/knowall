...
FileInfo_CSV = None  # Set empty for now - to be set in get_data_ext()
...
def check_input_file_type():
    ...
    Return string 'json' or 'csv' or raise error
    """
    # Check/Handle Input file Format to JSON
    try:  # Read first line and check if JSON
        json.loads(sys.stdin.readline())
        sys.stdin.seek(0)  # Reset back to line 0 before returning
        return "json"
    except:  # If lines aren't JSON, then try CSV
        try:
            # Get header for namedtuple
            sys.stdin.seek(0)  # Reset back to line 0 if JSON failed
            header = sys.stdin.readline().split(",")
            sys.stdin.seek(0)  # Reset back to line 0 before returning
            if "parent" in header and "folder" in header:
                return "csv"
            else:  # Doesn't contain expected CSV headers
                print("...Error...Input file is not JSON or CSV")
                raise
        except:  # Isn't JSON or CSV
            print("...Error...Input file is not JSON or CSV")
            raise
    ...
    if input_file_type == "json":  # Process JSON data
        for line in sys.stdin:
            ...
            if data["files"] or not filtered:
                yield data
    elif input_file_type == "csv":  # Process CSV data
        # Get header for namedtuple
        header = next(sys.stdin)
        # Rename header
        header = (
            header.replace("uid", "st_uid")
            .replace("size", "st_size")
            .replace("atime", "st_atime")
            .replace("mtime", "st_mtime")
            .replace("ctime", "st_ctime")
        )

        # Set named tuple (new CSV global value)
        global FileInfo_CSV
        ...
        file_list = []
        for line in sys.stdin:
            line = FileInfo_CSV._make(line.replace("\n", "").split(","))
            # General fixes to input values
            line = line._replace(
                # Fix path - backslash issues and network path
                path=line.path.replace(r"([^\\])\\([^\\])", r"\1\\\\\2").replace(
                    r",(\s*])", r"\1"
                ),
                # .replace("\\\\aa", "\\\\\\\\aa", 1), #Actually don't have to fix here
                # Convert time columns from text to numeric (based on get_options() logic)
                st_atime=int((parse(line.st_atime) - EPOCH).total_seconds()),
                st_mtime=int((parse(line.st_mtime) - EPOCH).total_seconds()),
            ...
            if line.folder == "True":
                ...
                #     # At root, so need to append dir_list to file_list to return root folders
                ...
                # Fix filename to have extension
                line = line._replace(name=line.name + "." + line.extension)
                # Add file to list to eventually yield with parent folder path
                file_list.append(line)
    else:  # Isn't JSON or CSV
        print("...Error...Input file is not JSON or CSV")
        raise
...
def find_hash(dbpath, filepath, fileinfo, no_hash=False):
    ...
    def callback(read, filepath=filepath, fileinfo=fileinfo, __last=[time.time()]):
        if time.time() - __last[0] > 10:
            pct = 100 * read / int(fileinfo.st_size)
    ...
    for data in get_data(opt):
        for filedata in data["files"]:
            try:
                ...
                fileinfo = FileInfo_CSV(*filedata)
...
def dupes(opt):
    ...
    for hashed in get_dupes(opt):
        ...
        for (size, hash), files in hashed.items():
            size = int(size) or 0  # can be None for unstatable files
            files_n = len(files)
            assert files_n > 1
